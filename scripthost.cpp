...
public:
  static jsi::HostFunctionType getFunc() {
    auto func = [](jsi::Runtime& runtime, const jsi::Value&, const jsi::Value* args, size_t count) {
  ...
  jsi::Value get(jsi::Runtime &runtime, const jsi::PropNameID &name) override {
    ...
    if (methodName == "appendString") {
      return jsi::Function::createFromHostFunction(
        ...
        return jsi::String::createFromAscii(runtime, arguments[0].getString(runtime).utf8(runtime) + arguments[1].getString(runtime).utf8(runtime).c_str());
...
public:
  v8RuntimeTask(std::unique_ptr<v8runtime::Task> task) : task_(std::move(task)) {}
  void run() override {
    task_->run();
  }
private:
  std::unique_ptr<v8runtime::Task> task_;
};

class MyTaskRunnerAdapter : public v8runtime::TaskRunner {
public:
  MyTaskRunnerAdapter(std::shared_ptr<MyTaskRunner> taskRunner)
    : taskRunner_(std::move(taskRunner)){}
  
  void postTask(std::unique_ptr<v8runtime::Task> task) override {
    taskRunner_->PostTask(std::make_unique<v8RuntimeTask>(std::move(task)));
  }

  void postDelayedTask(std::unique_ptr<v8runtime::Task> task, double delay_in_seconds) override {
    taskRunner_->PostDelayedTask(std::make_unique<v8RuntimeTask>(std::move(task)), delay_in_seconds);
  }

  void postIdleTask(std::unique_ptr<v8runtime::IdleTask> task) override {
    std::abort();
  }
...
ScriptHost::ScriptHost() {
  ...
  jsiTaskRunner_->PostTask(std::make_unique<FunctionTask>([this]() {
    ...
    //args.loggingCallback = [](const char* message, facebook::jsi::chakraruntime::LogLevel level) { std::cout << message << std::endl; };
    //runtime_ = facebook::jsi::chakraruntime::makeChakraRuntime(std::move(args));
  }));
  ...
  jsiTaskRunner_->PostTask(std::make_unique<FunctionTask>([this]() {
    ...
    /*runtime_->global().setProperty(
      *runtime_,
      "stringOps",
      jsi::Object::createFromHostObject(*runtime_, std::make_shared<StringOpHostObject>())
    );*/
    ...
    std::shared_ptr<jsi::HostObject> ho = stringOpsObj.getObject(*runtime_).getHostObject(*runtime_);
*/
	}));
}
...
void MyTaskRunner::PostDelayedTask(std::unique_ptr<MyTask> task, double delay_in_seconds) {
  {

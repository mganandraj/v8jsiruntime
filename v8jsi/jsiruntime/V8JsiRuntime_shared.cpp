...
V8Runtime::V8Runtime(V8RuntimeArgs&& args)
  : args_(std::move(args)) {
  ...
  if (!args_.platform) {
    ...
    // HACK!!
    // args_.inspector = InspectorInterface::create(const_cast<v8::Platform*>(platform_), 8080);
  } 
  ...
  if (args_.custom_snapshot_blob) {
    custom_snapshot_startup_data_ = { reinterpret_cast<const char*> (args_.custom_snapshot_blob->data()), static_cast<int>(args_.custom_snapshot_blob->size()) };
  ...
  auto foreground_task_runner = std::make_shared<TaskRunnerAdapter>(std::move(args_.foreground_task_runner));
  isolate_data_ = std::make_unique<v8runtime::IsolateData>(v8runtime::IsolateData({ std::move(foreground_task_runner), this }));
  ...
  if (args_.inspector) {
    args_.inspector->initialize(isolate_, isolate_->GetCurrentContext(), "JSIRuntime context");
...
jsi::Value V8Runtime::evaluateJavaScript(
  ...
  /*if (args_.cacheProvider) {
    ...
    std::unique_ptr<const jsi::Buffer> cache{ (*args_.cacheProvider)(sourceURL) };
  ...
  if (!v8::ScriptCompiler::Compile(context, &script_source, options).ToLocal(&script)) {
    ...
    if (!script->Run(context).ToLocal(&result)) {
	  ...
	  if (args_.preparedScriptStore && options != v8::ScriptCompiler::CompileOptions::kConsumeCodeCache) {
		  ...
		  args_.preparedScriptStore->persistPreparedScript(std::make_shared<ByteArrayBuffer>(codeCache->data, codeCache->length), scriptSignature, runtimeSignature, "perf");
	  }

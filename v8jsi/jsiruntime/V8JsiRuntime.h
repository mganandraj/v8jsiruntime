...
namespace v8runtime {
	...
	struct JSITaskRunner {
		...
		virtual void postDelayedTask(std::unique_ptr<JSITask> task, double delay_in_seconds) = 0;
	...
	__declspec(dllexport) std::unique_ptr<facebook::jsi::Runtime> __cdecl makeV8Runtime(V8RuntimeArgs&& args);

...
public:
	InspectorAgentDelegate(AgentImpl* agent, const std::string& script_path, const std::string& script_name, bool wait);
...
class AgentImpl {
public:
	explicit AgentImpl(v8::Platform* platform, int port);
	~AgentImpl();

  void initialize(v8::Isolate* isolate, v8::Local<v8::Context> context, const char* context_name /*must be null terminated*/);
...
private:
	...
	bool AppendMessage(MessageQueue* vector, int session_id, std::unique_ptr<v8_inspector::StringBuffer> buffer);
...
public:
  static std::unique_ptr<v8runtime::InspectorInterface> create(v8runtime::V8Platform& platform, int port, bool wait);
};
...
public:
  ...
  void setupContext(v8::Local<v8::Context> context, const char* context_name /*must be null terminated*/) {
    v8_inspector::V8ContextInfo info(context, 1, Utf8ToStringView(context_name)->string());
  ...
  bool isWaitingForResume() { return waiting_for_resume_; }
...
AgentImpl::AgentImpl(v8::Platform* platform, int port):
	platform_(platform),
  port_(port),
	wait_(false),
...
void InspectorConsoleCall(const v8::FunctionCallbackInfo<v8::Value>& info) {
	...
	if (!in_call) {
		CHECK(config_object->Set(context, in_call_key, v8::True(isolate)).FromJust());
		CHECK(!inspector_method.As<v8::Function>()->Call( context, info.Holder(), call_args.size(), call_args.data()).IsEmpty());
	}

	v8::TryCatch try_catch(info.GetIsolate());
	static_cast<void>(node_method.As<v8::Function>()->Call(context, info.Holder(), call_args.size(), call_args.data()));
...
void InspectorWrapConsoleCall(const v8::FunctionCallbackInfo<v8::Value>& args) {
	...
	args.GetReturnValue().Set(v8::Function::New(v8::Isolate::GetCurrent()->GetCurrentContext(), InspectorConsoleCall, array).ToLocalChecked());
}

void AgentImpl::initialize(v8::Isolate* isolate, v8::Local<v8::Context> context, const char* context_name /*must be null terminated*/) {
  isolate_ = isolate;
...
void AgentImpl::Start() {
  std::thread([this]() {
    ...
    InspectorSocketServer server(std::unique_ptr<InspectorAgentDelegate>(&delegate), port_);
    server_ = &server;
...
void AgentImpl::waitForDebugger() {
  ...
  v8_inspector::StringView reason(reinterpret_cast<const uint8_t*>(reasonstr.c_str()), reasonstr.size()), details(reinterpret_cast<const uint8_t*>(reasonstr.c_str()), reasonstr.size());
...
void AgentImpl::PostIncomingMessage(int session_id,
	...
	if (AppendMessage(&incoming_message_queue_, session_id, Utf8ToStringView(message))) {
		platform_->CallOnForegroundThread(isolate_, new DispatchOnInspectorBackendTask(this));        
...
void AgentImpl::WaitForFrontendMessage() {
  std::unique_lock<std::mutex> lock(incoming_message_cond_m_);
	if (incoming_message_queue_.empty())
		incoming_message_cond_.wait(lock, [this] {return !incoming_message_queue_.empty(); });
}
...
Agent::Agent(v8::Platform* platform, int port) : impl(new AgentImpl(platform, port)) {}

Agent::~Agent() {
	delete impl;
}

void Agent::initialize(v8::Isolate* isolate, v8::Local<v8::Context> context, const char* context_name /*must be null terminated*/) {
  impl->initialize(isolate, context, context_name);
}
...
namespace v8runtime {
std::unique_ptr<InspectorInterface> InspectorInterface::create(v8::Platform* platform, int port) {
  auto ptr = std::make_unique<inspector::Agent>(platform, port);
  ptr->start();
  return ptr;
}
}